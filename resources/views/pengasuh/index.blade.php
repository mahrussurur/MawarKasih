...
<div class="p-6 bg-gray-100 min-h-screen">
    <div class="flex justify-between mb-4">
        <button class="bg-blue-500 text-white px-4 py-2 rounded-lg flex items-center hover:bg-blue-600 transition shadow-sm">
            <a href="{{ route('pengasuh.create') }}">
                <span class="mr-2">+</span>Tambah Pengasuh
            </a>
        </button>
        <div class="flex gap-2">
            <a href="{{ route('pengasuh.cetak') }}" target="_blank" class="bg-white rounded-lg flex items-center hover:bg-gray-50">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-4 border">{{ $item->jabatan }}</td>
                    <td class="p-4 border text-center">{{ $item->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</td>
                    <td class="p-4 border">{{ $item->no_hp }}</td>
                    <td class="p-4 border text-center">
                        <div class="flex flex-col gap-2 items-center">
                            <a href="{{ route('pengasuh.show', $item->id) }}" class="bg-green-600 text-white px-4 py-1 rounded w-24 text-sm inline-block">
                                Lihat
                            </a>
                            <a href="{{ route('pengasuh.edit', $item->id) }}" class="bg-orange-400 text-white px-4 py-1 rounded w-24 text-sm inline-block">
                                Edit
                            </a>
...
function openDeleteModal(id) {
    document.getElementById('deleteForm').action = '/pengasuh/' + id;
